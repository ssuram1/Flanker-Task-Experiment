...
const Screen1 = ({ onButtonClick }) => (
  ...
  >
    ...
    >
      Welcome! <br /> <br />
      Before beginning the experiment, <br /> please read the instructions
      carefully. <br /> <br />
...
const Screen2 = ({ onButtonClick }) => (
  ...
  >
    ...
    >
      ...
      >
        A set of five faces will be shown in the center of the screen.
        <br />
        Your task will be to indicate the emotionality of the MIDDLE face.
        <br /> <br />
        To submit your answer, press one of the following keys: <br />
        Q if the MIDDLE face is NOT EMOTIONAL.
        <br />
        P if the MIDDLE face is EMOTIONAL.
        <br /> <br />
...
const Screen3 = ({ onButtonClick }) => (
  ...
  >
    ...
    >
      ...
      >
        Sometimes the surrounding faces will have different emotionality <br />{" "}
        than the face in the middle. <br /> <br />
        Only pay attention to the MIDDLE face!
        <br /> <br />
        Press Continue to read further instruction.
      </Typography>
        ...
        >
          {[...Array(5)].map((_, index) => (
            <img
              ...
              src={
                index === 2
                  ? "/RADIATE_BMP/RADIATE_450_COLOR_BMP/WF/WF01/WF01_AC.jpg"
                  : "/RADIATE_BMP/RADIATE_450_COLOR_BMP/WF/WF01/WF01_NC.jpg"
              }
  ...
  >
    ...
    >
      ...
      >
        ...
        >
          You will now be given a chance to practice
          <br /> before the test begins. <br /> <br />
          Remember: Keep focusing on the fixation point in the center of the
          screen and answer as quickly as possible but avoid mistakes. <br />
          <br />
...
const Screen6 = ({ onButtonClick }) => (
  ...
  >
    ...
    >
      ...
      >
        <Typography sx={{ fontSize: "100px", color: "white" }}>Q</Typography>
        <img
          src="neutral_face.png"
          alt="neutral face"
          style={{
      ...
      >
        <Typography sx={{ fontSize: "100px", color: "white" }}>P</Typography>
        <img
          src="angry_face.png"
          alt="angry face"
          style={{
...
const Screen9 = ({
  ...
  useEffect(() => {
    if (currentPattern1?.images) {
      const preload = async () => {
        const imagePromises = currentPattern1.images.map((src) => {
  ...
  return (
    ...
    >
      {!imagesRendered ? (
        ...
        >
          {currentPattern1?.images &&
            Array.isArray(currentPattern1.images) &&
            currentPattern1.images.map((imageUrl, index) => (
              <Box
                key={index}
                component="img"
                src={`${imageUrl}?nf_resize=fit&w=500`}
...
const Screen11 = ({ onButtonClick, value, onChange, PID }) => (
  ...
  >
    ...
    >
      ...
      >
        Thank you for completing the study, please enter the following
        confirmation code for completion back on redcap.case.edu and/or
        Prolific:
      </Typography>
...
const preloadImages = async (images) => {
  console.warn(`Preloading ${images.length} images`);
...
const FaceExperiment = ({ experiment, PID }) => {
        ...
        Papa.parse(response.data, {
          ...
          complete: async (results) => {
            ...
            results.data.forEach((row) => {
              if (row.target) uniqueImages.add(row.target.trim());
              if (row.flanker)
                row.flanker
                  .split(";")
                  .forEach((img) => uniqueImages.add(img.trim()));
            });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        Papa.parse(response.data, {
          header: true, //execute complete callback when parsing is completed and take results as arg
          complete: (results) => {
            console.log("Parsed data:", results.data); //store pattern in parsedPatterns
            const parsedPatterns = results.data //ensures flanker and target values are present- filters out missing rows
              .filter((row) => row.flanker && row.target) //transforms each row and returns new array with pattern
              .map((row) => {
            ...
            const correctAnswer = results.data
              .filter((row) => row.correct) //makes entire correct column uppercase
              .map((row) => row.correct.toUpperCase()); //store congruency of each pattern
            const congruent = results.data
              .filter((row) => row.congruency)
              .map((row) => row.congruency); //stores faces type for each pattern
            const faces = results.data
              .filter((row) => row.faces)
              .map((row) => row.faces);
            console.log("Parsed patterns:", parsedPatterns); //parsedPatterns contains images array for each pattern
  ...
  useEffect(() => {
    if (screen === 5 || (screen === 1 && continued == true)) {
      const handleStart = (event) => {
        console.log(screen); //press space to advance to patterns
        if (event.code == "Space") {
          console.log(screen); //move to experiment
      ...
      window.addEventListener("keydown", handleStart); //clean up event listener
      return () => {
  ...
  const handleSpace = (event) => {
    ...
    if ((tooFast || tooSlow) && event.code === "Space") {
      ...
      console.log(currentPatternIndex); //if every 50 trials, set takebreak to true //change to 58
      ...
      if (tooFast == true) {
        ...
        saveResponse(null, 2000); // hides the too slow message
        setTooSlow(false);
      }
  ...
  useEffect(() => {
    let slow, move; //stores start time for pattern
    if (screen === 9 && currentPatternIndex >= 8) {
      setStartTime(Date.now());
      console.log(startTime); //waits 2 seconds, then says too slow and adds event listener for space bar
      slow = setTimeout(() => {
        if (!tooFast) {
          setTooSlow(true); //add event listener for user to hit space bar
  ...
  const movePatternToEnd = () => {
    console.log("Moving Pattern to Back"); //replace patterns with new rearranged array //takes previous state(prevSkipped) as arg and returns new state as array with currPatternIndex
    setPatterns((prevPatterns) => {
      //copy current array from arg prevPatterns
      const updatedPatterns = [...prevPatterns]; //remove current pattern from array and store pattern
      const currentPattern = updatedPatterns[currentPatternIndex]; //adds pattern to end of array
    ...
    setCorrectAnswers((prevCorrectAnswers) => {
      ...
      const updatedCorrectAnswers = [...prevCorrectAnswers]; //remove current answer from array and store answer
      const currentCorrectAnswer = updatedCorrectAnswers[currentPatternIndex]; //adds correct answer to end of array
    ...
    setCongruency((prevCongruency) => {
      ...
      const updatedCongruency = [...prevCongruency]; //remove current congruency from array and store answer
      const currentCongruency = updatedCongruency[currentPatternIndex]; //adds correct congruency to end of array
    ...
    setFaces((prevFaces) => {
      ...
      const updatedFaces = [...prevFaces]; //remove current faces from array and store answer
      const currentFaces = updatedFaces[currentPatternIndex]; //adds correct faces to end of array
  ...
  useEffect(() => {
    const handleKeyDown = (event) => {
      ...
      const { key } = event; //Experiment Block //only process keydown if no key is currently pressed
      if (
        !tooSlow &&
        !tooFast &&
        !keyPressed &&
        (key === "Q" || key === "P" || key === "q" || key === "p")
      ) {
        //since key has been assigned, key has been pressed
        setKeyPressed(true); //if experiment trial
        if (screen === 9 && currentPatternIndex >= 8) {
          const endTime = Date.now();
          const duration = endTime - startTime;
          console.log(startTime);
          console.log(endTime);
          console.log("Response duration:", duration); //if too fast response, mark as incorrect and send to end of experiment
          if (duration < 150) {
            ...
            saveResponse(key.toUpperCase(), duration);
            setCurrentPatternIndex((prevIndex) => prevIndex + 1); //if every 50 trials, set takebreak to true //change to 58 for full experiment
            if (currentPatternIndex % 58 == 0) {
              setTakeBreak(true);
            }
          ...
          screen === 9 &&
          key.toUpperCase() == correctAnswers[currentPatternIndex]
        ) {
          setCurrentPatternIndex((prevIndex) => prevIndex + 1);
          setInputValue("");
          setScreen(6);
  ...
  }, [screen, stopwatch, tooSlow, keyPressed, tooFast]); //useEffect to listen for space when tooFast
  ...
  }, [tooFast]); //creates new response object with screen number, response, and time

  const saveResponse = (response, duration) => {
    const existingResponses =
      JSON.parse(localStorage.getItem("responses")) || []; //set trial number
    let trialNumber = currentPatternIndex; //when currentPatternIndex is greater than total number of trials //change based on total number of trials!!!
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (screen === 8) {
        setDelay(1000); //when hit currentPatternIndex = 58, give 10 sec break
        if (
  ...
  }, [screen, delay]); //downloads responses when reach last screen
  ...
  }, [screen]); //rotates instruction screens 1-5 and experiment screens
  ...
  const downloadResponses = () => {
    ...
    a.href = url; //custom download name : ID_experiment.json
    a.download = `${PID}_${experiment}.json`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
    sendDataToREDCap(jsonData); // Clear out local storage
    localStorage.removeItem("responses");
  };

  //creates json file and sends to RedCap
  const sendDataToREDCap = (data) => {
    console.log("Sending responses to RedCap"); //creates URLSearchParams API object to extract parameters
    const urlParams = new URLSearchParams(window.location.search); //retrieve PID from URL query string
    const userid = PID || "10";
    let url = "";
    console.log(typeof userid); //REDCap API endpoint
    if (experiment === "F1" || experiment === "A1") {
      url = "https://redcap.case.edu/api/";
    } else {
      url = "https://redcap.case.edu/api/";
    }
    console.log(url); // Determine which field to use based on the experiment
    ...
    const body = {
      ...
      data: JSON.stringify([
        {
          record_id: userid, //change for faces as well
          [dataField]: data, //make flanker_data_json_2 for F1 and A3
  ...
  return (
    <div>
            
      {screen === 1 && (
        <Screen1 continued={continued} onButtonClick={() => switchScreen(2)} />
      )}
      ...
      {screen === 9 && (
        <Screen9
          value={inputValue}
          onChange={(e) => setInputValue(e.targscet.value)}
